...
def compute_loss(pred, gt):
    ...
    quat_gt_conj = quaternion_conjugate_torch(quat_gt)

    q_pred_inv = torch.cat((quat_pred[:, 0:1], -quat_pred[:, 1:]), dim=1)

    # Compute the difference between the two quaternions
    q_diff = quaternion_product(quat_gt, q_pred_inv)

    quat_loss = quaternion_log(q_diff)
    quat_loss = quat_loss.norm(dim=1).pow(2).mean()
...
def train_one_epoch(model, loader, optimizer):
    ...
    return total_loss / len(loader), pos_total_loss / len(loader), quat_total_loss/ len(loader)
...
def main():
    ...
    # You are using the same dataset for train and val here (just for testing)
    train_dataset = IMUDatasetFromMat("trajectories_3.mat")
    ...
    for epoch in range(num_epochs):
        ...
        train_loss, train_pos_loss, train_quat_loss = train_one_epoch(model, train_loader, optimizer)
        val_loss, val_pos_loss, val_quat_loss = validate(model, val_loader)

        print(f"Train Loss: {train_pos_loss:.4f} | Train Pos Loss: {train_pos_loss:.4f}")
        print(f"Train Loss: {train_quat_loss:.4f} | Train Quat Loss: {train_quat_loss:.4f}")
        print(f"Val   Loss: {val_pos_loss:.4f} | Val   Pos Loss: {val_pos_loss:.4f}")
        print(f"Val   Loss: {val_quat_loss:.4f} | Val   Quat Loss: {val_quat_loss:.4f}")
